...
function __promptline_left_prompt {
  ...
  [ $is_prompt_empty -eq 1 ] && slice_prefix="$slice_empty_prefix"
  # section "a" slices
  __promptline_wrapper "snoop" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; is_prompt_empty=0; }
...
function __promptline {
  ...
  local warn_bg="${wrap}48;5;214${end_wrap}"
  local warn_sep_fg="${wrap}38;5;214${end_wrap}"
  if [[ -n ${ZSH_VERSION-} ]]; then
